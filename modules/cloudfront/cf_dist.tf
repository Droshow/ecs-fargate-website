...
resource "aws_cloudfront_origin_access_identity" "ghost_distribution" {
...
resource "aws_cloudfront_distribution" "ghost_distribution" {
  origin {
    domain_name = aws_s3_bucket.ghost_bucket.bucket_regional_domain_name
    origin_id   = "${var.site_name}_ghostBucket"

    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.ghost_distribution.cloudfront_access_identity_path
    }
  }
  enabled             = true
  is_ipv6_enabled     = true
  comment             = "${var.site_name} Distribution for ghost"
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${var.site_name}_ghostBucket"
    compress         = true
...
resource "aws_s3_bucket_policy" "ghost_bucket" {
  bucket = aws_s3_bucket.ghost_bucket.id
      ...
      "Statement" : [
        {
          ...
          "Effect" : "Allow",
          "Principal" : {
            "AWS" : aws_cloudfront_origin_access_identity.ghost_distribution.iam_arn
          },
          "Action" : "s3:GetObject",
          "Resource" : "${aws_s3_bucket.ghost_bucket.arn}/*"
        }
      ]
    }
  )
