resource "random_password" "serverless_ghost_password" {
  length           = 16
  special          = true
  override_special = "!#%&*()-_=+[]<>"
}
...
resource "aws_security_group" "aurora_serverless_group" {
  ...
  description = "security group for serverless ghost mysql aurora"
...
#tfsec:ignore:AWS089
resource "aws_cloudwatch_log_group" "serverless_ghost" {
  name              = "/aws/rds/cluster/${var.site_name}-serverless-ghost/error"
  retention_in_days = 7
}

resource "aws_rds_cluster" "serverless_ghost" {
  ...
  cluster_identifier                  = "${var.site_name}-serverless-ghost"
  engine                              = "aurora-mysql"
  ...
  master_password                     = random_password.serverless_ghost_password.result
  ...
  final_snapshot_identifier = "${var.site_name}-serverless-ghost-${random_id.rds_snapshot.dec}"
  snapshot_identifier       = var.snapshot_identifier
  depends_on                = [aws_cloudwatch_log_group.serverless_ghost]
}
